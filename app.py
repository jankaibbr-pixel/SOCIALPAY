...
EMAIL_PASS = "ngca qknw rumj mxue"   # ← updated App Password
...
def send_email(to_email, subject, html_body):
    ...
    try:
        ...
        print(f"[EMAIL ERROR] Recipient refused ({to_email}): {e}")
        return False
...
def register():
    ...
    if request.method == "POST":
        ...
        if not sent:
            # SMTP failed — do NOT bypass OTP. Block registration and inform user.
            log_audit("register_email_failed", uid)
            print(f"[REGISTER] OTP email failed for {email} — registration blocked.")
            return jsonify({
                "success": False,
                "message": "Unable to send verification email. Please check your email address and try again, or contact support."
            })
